        ...
        <div>
          <ul ref={pages} className="pagination mt-2 d-flex justify-content-center">
            {pageNumbers.map(number => (
                <li onClick={paginate}  id={number} key={number} className={number === lastActivePage ? "page-item border-0 active": "page-item border-0"}><a className="page-link shadow-none" href="#">{number}</a></li>
            ))}
        </ul>  
        </div>
    )
